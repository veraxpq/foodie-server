...
public class ControllerExceptionHandler {
    ...
    @ResponseBody
    public ResponseEntity<ErrorMessage> handlePermissionDenyException(
            PermissionDenyException e, HttpServletRequest request, HttpServletResponse response) {
        logger.debug(ExceptionUtils.getStackTrace(e));
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json;charset=utf-8");
        return new ResponseEntity<ErrorMessage>(new ErrorMessage(1010, e.getMessage()), headers, HttpStatus.FORBIDDEN);
    }
